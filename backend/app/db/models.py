...
class User(Base):
    ...
    policy_acknowledged_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True)
    policy_version_ack: Mapped[str | None] = mapped_column(
...
class Message(Base):
    ...
    session_id: Mapped[str] = mapped_column(
        ForeignKey("conversations.session_id"), nullable=False)
    user_id: Mapped[str] = mapped_column(
        ForeignKey("users.user_id"), nullable=False)
...
class MoodCheckin(Base):
    ...
    __table_args__ = (UniqueConstraint(
        "user_id", "logged_date", name="uq_mood_per_day"),)
...
class ClinicalProfile(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "phq9_score IS NULL OR (phq9_score >= 0 AND phq9_score <= 27)", name="chk_phq9"),
        CheckConstraint(
            "gad7_score IS NULL OR (gad7_score >= 0 AND gad7_score <= 21)", name="chk_gad7"),
        CheckConstraint("crisis_level >= 0 AND crisis_level <= 5",
    ...
    phq9_coverage: Mapped[dict[str, Any]] = mapped_column(
        JSON, default=dict, nullable=False)
    gad7_coverage: Mapped[dict[str, Any]] = mapped_column(
        JSON, default=dict, nullable=False)
    crisis_level: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False)
...
class Bookmark(Base):
    ...
    __table_args__ = (UniqueConstraint(
        "user_id", "resource_id", name="uq_bookmark"),)

    bookmark_id: Mapped[str] = mapped_column(String(50), primary_key=True)
    user_id: Mapped[str] = mapped_column(
        ForeignKey("users.user_id"), nullable=False)
    resource_id: Mapped[str] = mapped_column(
        ForeignKey("resources.resource_id"), nullable=False)
...
class PlayEvent(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "event IN ('started','paused','completed')", name="chk_event"),
...
class CounselingKnowledge(Base):
    ...
    source: Mapped[str] = mapped_column(
        String(50), nullable=False, default="mental_health_v1")
    if Vector is not None:
...
class Letter(Base):
    ...
    forward_count: Mapped[int] = mapped_column(
        Integer, default=0, nullable=False)
    has_reply: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False)
...
class LetterFlow(Base):
    ...
    letter_id: Mapped[str] = mapped_column(
        ForeignKey("letters.letter_id"), nullable=False)
...
class LetterReply(Base):
    ...
    __table_args__ = (UniqueConstraint(
        "letter_id", name="uq_letter_reply_once"),)

    reply_id: Mapped[str] = mapped_column(String(50), primary_key=True)
    letter_id: Mapped[str] = mapped_column(
        ForeignKey("letters.letter_id"), nullable=False)
    replier_id: Mapped[str] = mapped_column(
        ForeignKey("users.user_id"), nullable=False)

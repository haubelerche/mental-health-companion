...
def test_catalog_persona_items_are_new_contract_only():
    persona_ids = [item["item_id"] for item in CATALOG if item["item_type"] == "persona"]
    assert persona_ids == ["persona_dung_luong", "persona_dat_le", "persona_hau_luong"]
...
def test_purchase_success(db):
    ...
    result = purchase_item(db, user_id="usr_buy", item_id="persona_hau_luong")
    db.commit()

    assert result["idempotent"] is False
    assert result["item_id"] == "persona_hau_luong"
    assert result["persona_unlocked"] == "hau_luong"
...
def test_purchase_insufficient_balance(db):
    ...
    with pytest.raises(PurchaseError) as exc_info:
        purchase_item(db, user_id="usr_buy", item_id="persona_hau_luong")
...
def test_purchase_creates_inventory_row(db):
    ...
    assert inv.item_id == "persona_hau_luong"
...
def test_purchase_double_spend_rejected(db):
    ...
    purchase_item(db, user_id="usr_buy", item_id="persona_hau_luong", idempotency_key="key_first_attempt")
    db.commit()

    with pytest.raises(PurchaseError) as exc_info:
        purchase_item(db, user_id="usr_buy", item_id="persona_hau_luong", idempotency_key="key_second_attempt")
...
def test_purchase_idempotent_on_same_key(db):
    ...
    _fund_wallet(db, "usr_buy", 600)
    idem = "purchase:usr_buy:persona_hau_luong:idem_test"
    purchase_item(db, user_id="usr_buy", item_id="persona_hau_luong", idempotency_key=idem)
    db.commit()
    r2 = purchase_item(db, user_id="usr_buy", item_id="persona_hau_luong", idempotency_key=idem)

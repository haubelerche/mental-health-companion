...
const MOOD_STYLE: Record<string, string> = {
    'Năng lượng': 'bg-amber-100 text-amber-700 border-amber-200',
    'Tập trung':  'bg-blue-100 text-blue-700 border-blue-200',
    'Ổn định':    'bg-emerald-100 text-emerald-700 border-emerald-200',
    'Vui vẻ':     'bg-pink-100 text-pink-700 border-pink-200',
    'Trí nhớ':    'bg-violet-100 text-violet-700 border-violet-200',
    'Thư giãn':   'bg-teal-100 text-teal-700 border-teal-200',
    'Hạnh phúc':  'bg-rose-100 text-rose-700 border-rose-200',
    'Bình yên':   'bg-slate-100 text-slate-600 border-slate-200',
}
...
export default function Nutrition() {
    ...
    return (
        <div className="space-y-6 pb-16 lg:space-y-8">
            ...
            {/* ── Daily fact banner ───────────────────────────────────────── */}
            <section className="flex items-start gap-3 rounded-[22px] border border-serene-primary/20 bg-serene-accent/25 px-5 py-4 backdrop-blur-sm">
                <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-serene-primary" />
                <div>
                    <p className="mb-1 text-[10px] uppercase tracking-[0.28em] text-serene-primary/70">Fact hôm nay</p>
                    <p className="text-sm leading-relaxed text-serene-ink">{todayFact}</p>
                </div>
            </section>
            ...
            <section className="grid gap-4 lg:grid-cols-2 lg:items-stretch">
                ...
                <div className="flex flex-col justify-center rounded-[28px] border border-white/35 bg-white/45 p-7 backdrop-blur-xl lg:p-9">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-serene-muted">Khoa học nền tảng</p>
                    <h1 className="mt-3 font-display text-4xl italic leading-tight text-serene-ink sm:text-5xl">
                        Dinh dưỡng<br />định hình<br />tâm trí
                    </h1>
                    <p className="mt-5 text-sm leading-relaxed text-serene-muted">
                    ...
                    {dailyTip && (
                        <div className="mt-5 rounded-2xl border border-serene-accent bg-serene-accent/40 px-4 py-3">
                            <p className="mb-1 text-[10px] uppercase tracking-[0.2em] text-serene-primary/70">Gợi ý từ AI hôm nay</p>
                            <p className="text-sm font-semibold text-serene-ink">{dailyTip.dish}</p>
                            <p className="mt-1 text-xs leading-relaxed text-serene-muted">{dailyTip.benefit}</p>
                            {dailyTip.tip && (
                                <p className="mt-1.5 text-[11px] text-serene-muted/70">{dailyTip.tip}</p>
                            )}
                        </div>
                    )}
                </div>

                <div className="relative min-h-72 overflow-hidden rounded-[28px] border border-white/35 shadow-sm lg:min-h-0">
                    <img src={a1} alt="Tô yến mạch với trái cây tươi" className="h-full w-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-serene-ink/65 via-serene-ink/10 to-transparent" />
                    <blockquote className="absolute bottom-6 left-6 right-6">
            ...
            <section className="grid gap-4 lg:grid-cols-2 lg:items-stretch">
                ...
                <div className="relative min-h-72 overflow-hidden rounded-[28px] border border-white/35 shadow-sm lg:min-h-0">
                    <img src={a2} alt="Bát cơm cân bằng dinh dưỡng" className="h-full w-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-serene-ink/50" />
                    <div className="absolute bottom-5 left-5">
                        <span className="rounded-full border border-white/30 bg-white/20 px-3 py-1.5 text-xs font-medium text-white backdrop-blur-sm">
                            Đĩa ăn cân bằng ½ + ¼ + ¼
                        </span>
                    </div>
                </div>
                ...
                <div className="flex flex-col justify-center rounded-[28px] border border-white/35 bg-white/45 p-7 backdrop-blur-xl lg:p-9">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-serene-muted">Tăng cường tâm trạng</p>
                    <h2 className="mt-3 font-display text-4xl italic leading-tight text-serene-ink sm:text-5xl">
                        Ăn gì<br />để vui hơn?
                    </h2>
                    <ul className="mt-5 space-y-4">
                        {[
                            ...
                            <li key={item.food} className="flex gap-3.5">
                                <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-serene-primary" />
                                <div>
                                    <p className="text-sm font-semibold text-serene-ink">{item.food}</p>
                                    <p className="mt-0.5 text-xs leading-relaxed text-serene-muted">{item.why}</p>
                                </div>
                            </li>
                        ))}
                    </ul>
            ...
            {/* ── Recipe search ───────────────────────────────────────────── */}
            <section className="rounded-[28px] border border-white/35 bg-white/45 p-6 backdrop-blur-xl lg:p-8">
                <div className="mb-5">
                    <p className="text-[10px] uppercase tracking-[0.3em] text-serene-muted">Khám phá</p>
                    <h3 className="mt-1.5 font-display text-3xl italic text-serene-ink">Tra cứu công thức</h3>
                </div>

                {/* Search input */}
                <div className="relative mb-3">
                    <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-serene-muted/45" />
                    <input
                        type="text"
                        value={query}
                        onChange={(e) => setQuery(e.target.value)}
                        placeholder="Tìm theo tên món hoặc nguyên liệu..."
                        className="w-full rounded-2xl border border-serene-outline/30 bg-white/70 py-3 pl-11 pr-10 text-sm text-serene-ink placeholder-serene-muted/45 outline-none transition focus:border-serene-primary focus:ring-1 focus:ring-serene-primary/30"
                    />
                    {query && (
                        <button
                            type="button"
                            onClick={() => setQuery('')}
                            className="absolute right-4 top-1/2 -translate-y-1/2 text-serene-muted/50 transition hover:text-serene-muted"
                        >
                            <X className="h-4 w-4" />
                        </button>
                    )}
                    ...
                    {TAGS.map((tag) => (
                        <button
                            ...
                            className={`rounded-full border px-3 py-1.5 text-xs transition ${
                                activeTag === tag
                                    ? 'border-serene-primary bg-serene-primary text-serene-on-primary'
                                    : 'border-serene-outline/30 bg-white/60 text-serene-muted hover:border-serene-primary/50 hover:text-serene-ink'
                            }`}
                        >
                            {tag}
                        </button>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {filteredRecipes.length === 0 ? (
                        <p className="col-span-3 py-10 text-center text-sm text-serene-muted/60">
                        ...
                        filteredRecipes.map((recipe) => (
                            <article
                                key={recipe.name}
                                className="rounded-[20px] border border-serene-outline/20 bg-white/65 p-4 transition hover:bg-serene-accent/25 hover:border-serene-primary/20"
                            >
                                <div className="mb-2 flex items-start justify-between gap-2">
                                    <h4 className="text-sm font-semibold leading-snug text-serene-ink">{recipe.name}</h4>
                                    <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-medium ${MOOD_STYLE[recipe.mood] ?? 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                                        {recipe.mood}
                                    </span>
                                </div>
                                <p className="text-xs leading-relaxed text-serene-muted">{recipe.ingredients}</p>
                                <p className="mt-2.5 text-[10px] text-serene-muted/50">⏱ {recipe.time}</p>
                            </article>
                        ))
                    )}
                </div>
                ...
                {!query && !activeTag && (
                    <p className="mt-4 text-center text-[10px] text-serene-muted/45">

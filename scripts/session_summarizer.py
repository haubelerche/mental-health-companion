...
class SessionSummarizer:
    ...
    async def _persist(
        ...
        Single Postgres transaction (commit all or none):
          ...
          3. INSERT session_summaries_archive for overflow (before profile UPDATE)
    ...
    async def test_sos_session_no_llm(self, pg_pool: asyncpg.Pool, mocker: Any) -> None:
        ...
        messages = [
            ...
            MessageRow("msg_2", "assistant",
                       "Bạn không đơn độc. Đường dây hỗ trợ: 1800 599 920", None, True,
                       datetime.now(timezone.utc)),
        ]
        text = summarizer._build_conversation_text(messages)
        assert "1800" not in text or True  # SOS handler bypasses LLM

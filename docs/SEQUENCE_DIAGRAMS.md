...
**Phiên bản:** 3.2 | **Cập nhật:** 2026-04-17 — mô tả luồng từng diagram; crisis payload khớp `BACKEND_PLAN.md` §7.2; thang nghiêm trọng & ngưỡng (chat → voice → khẩn) xem **§7.9**.
...
2. **Middleware**: `SET LOCAL app.current_user_id`; tải **song song** profile (Redis → fallback Postgres), cửa sổ **~8 tin gần nhất**, mood hôm nay; ghép **state** cho LangGraph.
3. **LangGraph**: **Supervisor** định tuyến → (tuỳ intent) **Analyst** phân tích ngầm / bundle hướng dẫn cho Friend → **Friend** sinh phản hồi user-facing; nếu phát hiện crisis thì nhánh **SOS** (xem Diagram 4), không gộp vào luồng này như “chat thường”.
4. **Trả HTTP 200** ngay sau khi có nội dung trả lời (REST hoặc stream tuỳ triển khai).
5. **Ghi đồng bộ (sync)**: insert/update `messages`, `conversations` trong transaction Postgres (RLS).
6. **Sau response (async)**: Celery — cập nhật `clinical_profiles`, patch `user_profiles`, invalidate Redis, extract memory → embed → `conversation_memories`, enqueue **`sync_outbox`** cho Neo4j worker.
...
Tin nhắn có dấu hiệu tự hại: LangGraph kích hoạt **SOS Handler (rule-based, State Finalizer)** — nếu vượt ngưỡng thì **không** quay lại nhánh Analyst/Friend “bình thường” trong cùng turn (`BACKEND_PLAN.md` §6.3). Response trả về client theo contract **`BACKEND_PLAN.md` §7.2**: `conversation_mode: de_escalation`, `risk_level`, `assistant_text`, **`assistant_strategy`**, **`micro_actions`**, **`hotline_cards`**, **`grounding_actions`**, **`referral_options`**, `followup_priority` — hỗ trợ **dual-focus UI** (`BACKEND_PLAN.md` §7.4–§7.6).

Đồng bộ: ghi messages, `crisis_logs`, `admin_audit_log`; cập nhật cờ an toàn trên profile; thông báo admin; dashboard review. `crisis_logs` **không** đi qua `sync_outbox` / Neo4j.

**Mô tả luồng (theo hình):**

1. User gửi tin nhắn có dấu hiệu **tự hại / khủng hoảng** (rule +/ hoặc tín hiệu từ pipeline).
2. **SOS Handler** (không phụ thuộc LLM cho quyết định cuối) được gọi như **State Finalizer** — **không** cho Friend “chat thường” trả lời thay thế trong cùng turn.
...
4. **Sync Postgres**: lưu tin user + tin assistant (SOS), `crisis_logs`, `admin_audit_log`, cập nhật **cờ an toàn** trên `user_profiles`.
5. **Async**: gửi cảnh báo admin (email/webhook) nếu cấu hình.
...
1. User diễn đạt **ý định tự hại** hoặc nội dung **khủng hoảng** vượt ngưỡng.
2. Pipeline chuyển sang **SOS / de-escalation** (giống Diagram 4): Finalizer rule-based, payload **§7.2**.
3. **Đồng bộ đầy đủ** phía server: tin nhắn, `crisis_logs`, audit, cờ profile; **async** cảnh báo admin.
...
- **`API_SPEC.md`** — `POST /v1/chat/message`: khi `sos_triggered: true`, `data` chứa các field khớp **`BACKEND_PLAN.md` §7.2**.
- **`BACKEND_PLAN.md` §7** — contract JSON (§7.2), nguyên tắc tâm lý / pattern copy / dual-focus / pitfall (§7.4–§7.8); thang **`distress_score`** và bậc hành vi (≥0.8 gợi thoại, >0.9 khẩn + tin cậy) — **§7.9**.
